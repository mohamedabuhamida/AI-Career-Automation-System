...
class Experience(BaseModel):
    ...
    # ضفنا دي عشان نعرف نحسب سنين الخبرة
...
class Education(BaseModel):
    degree: str = Field(description="Degree name (e.g., Bachelor of Computer Science)")
    institution: Optional[str] = Field(description="University name")
    
    # ضفنا دي عشان نعرف هو خريج جديد ولا قديم
...
class CVStructured(BaseModel):
    ...
    skills: List[str] = Field(
        ...
        description="List of technical skills (Python, Docker, etc.) and soft skills"
    )
    
    experience: List[Experience] = Field(
        default_factory=list, 
        description="Work history ordered by date"
    )
    ...
    projects: List[str] = Field(
        default_factory=list, 
        description="List of project titles and brief descriptions"
    )
